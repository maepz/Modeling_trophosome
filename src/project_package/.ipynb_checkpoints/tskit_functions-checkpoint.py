...
def cell_coalescent_tree_with_mutations_v2(n, mu, fitness, growth_factor, tree_seed=None, mutation_seed=None, fitness_seed=None):
    ...
    demography.add_population(initial_size=float(n), growth_rate=math.log(growth_factor))

    ts = msprime.sim_ancestry(n, demography=demography, sequence_length=1, ploidy=1,random_seed=tree_seed)
...
def cell_coalescent_tree_with_mutations_v1(n, mu, fitness, tree_seed, mutation_seed, fitness_seed):
...
epsilon=1e-9,
    ...
    then prune & regraft terminals along the lineage which doesn't have mutations
